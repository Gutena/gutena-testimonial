...
const GutenaFontSizePicker = ( props ) => {
    const {
        label = __( 'Size', 'gutena-testimonial' ),
        fontSize = undefined,
        onChangeFunc = noop,
        reponsive=false,
        setCustom=false
    ...
    let customLabel =  customSize ? __( 'Custom', 'gutena-testimonial' ) : getMatchArrObjKeyValue( fonSizesArray, 'value', fontSize, 'label' );
    
    customLabel = ( false === customLabel || ( ! customSize &&  gkIsEmpty( fontSize ) ) ) ? __( 'Default', 'gutena-testimonial' ) : '('+ customLabel + ')';
    ...
    return (
        ...
        <>
            <HStack>
                ...
                <FlexItem>
                    <Button
                        label={ customSize ? __( 'Use size preset', 'gutena-testimonial' ) : __( 'Set custom size', 'gutena-testimonial' ) }
                        icon={  settings }
